...
function test_unsuccessful_assert_is_directory_readable_without_execution_permission() {
  if [[ "$_BASHUNIT_OS" == "Windows" || $_BASHUNIT_DISTRO = "Alpine" ]]; then
    return
  fi
...
function test_unsuccessful_assert_is_directory_not_writable() {
  ...
  assert_same "$(bashunit::console_results::print_failed_test "Unsuccessful assert is directory not writable" \
