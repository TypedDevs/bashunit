...
### Changed
- `bashunit bench` follows the same rule as `bashunit test` for a path that does not exist: it is named and refused, rather than reported as `No benchmarks found`. A file that exists but holds no `bench_` function is still an empty selection and keeps that message. Both subcommands share one check, since #1199 closed exactly the gap where they disagreed about a missing path (#1263)
...
- Performance: per-**file** cleanup no longer forks `rm` when the file created no temp file, the script-level twin of the change below — about 3.9ms per file, measured as 8.58s to 7.81s over 200 files, interleaved in one tree (#1271)
- Performance: the total runtime is now computed in the shell instead of through `bc`, saving a fork and a subshell per run. Every clock reading is already an integer nanosecond count, so nothing needed the arbitrary-precision path — and staging that pipe is what could fail (#1271)
...
### Fixed
- A sequential run no longer leaks a file descriptor per test file. Bash 3.x reaps the descriptors process substitution allocates only at the top level or after forking an external command, and the per-file scan runs from inside a function the run never leaves — so the per-file `rm` fork removed just above had been the only thing collecting them. At the limit the run stopped executing tests and reported `risky` rather than failing: 120 files under a 120-descriptor limit ran 2 assertions instead of 120, and still exited 0 (#1271)
- `Time taken` no longer prints a negative duration when a clock reading comes back smaller than the run's start (#1271)
- `Time taken` no longer reads `0ms` when the runtime could not be measured. The empty result met a `${time:-0}` default and rendered as a real instant run, so a failed measurement was indistinguishable from a fast suite; it now says `unknown`, which also covers a genuinely unavailable clock (#1271)
