...
- Docs: the project overview told new contributors to run benchmarks with `-s -b` / `--simple --bench`, neither of which exists — both are rejected as unknown options. Benchmarks are the `bashunit bench [path]` subcommand (#1227)
- Docs: the drop-in agent rules described `assert_equals` as trimming or normalizing. It strips ANSI colour codes, tabs and newlines — not spaces, so `assert_equals "a" " a "` fails. That block is written to be pasted verbatim into an `AGENTS.md`, so the error propagated into every repo adopting it (#1225)
...
- Performance: `./build.sh` is about 1.9x faster (7.6s to 4.0s). Comment stripping round-trips the artifact through shfmt's JSON AST, and `jq`'s `walk` — defined in jq's own language, so it rebuilds every node of a 57 MB AST — cost 5.1s of a 6.5s strip, more than the parse and re-print combined. A targeted path update plus `jq -c` produces a byte-identical artifact. Pays on every `Build & Verify` job, every release, and the build test (#1233)
...
- `bashunit --output junit` produces valid XML. A blank line printed between a file's tests and the next landed in front of the document, so the declaration was not first and no parser accepted it — and since parallel is not the default, the plain documented command was the broken one. `--output json` gained the same leading blank line, unnoticed because JSON tolerates leading whitespace (#1243)
...
- A piped `--parallel` run no longer emits a stray `\r  \r`. The spinner is erased once the aggregation ends, but it only *draws* on a terminal, so erasing it unconditionally wrote those bytes into every piped run — which is every CI log. The erase now checks the same conditions the spinner checks before drawing
- An empty `--parallel` run renders its notice on its own line instead of `No tests found  Tests:      0 total`: the per-script "no result files" notice was not newline-terminated, so it ran into the totals line the summary prints next
...
- A coverage run that tracked no executable line reports `no executable lines were tracked` and names `--coverage-paths`, instead of `Coverage 0% is below minimum N%` — that 0% is arithmetic, not a measurement. The gate still fails (#1171)
- A run's scratch-directory cleanup can no longer widen to every concurrent run's: the safety guard accepted the per-OS parent that all runs on a machine share, so a run whose random token came out empty would have deleted the others' directories too (#1165)
- A run survives its scratch directory going missing and says so once on stderr, naming the directory, instead of reporting a valid test file as unsourceable — the redirect was failing, not the file (#1163, #1167)
- Duplicate test functions are detected under `--parallel` again: the run reported "All tests passed" and exited 0 over a file where one of two same-named tests never ran. The report also names the line of every definition (#1147)
- A `@data_provider` naming a function that is not defined, or one that yields no data, is reported as an error naming the provider instead of making the test vanish behind "No tests found". Under `--parallel` the failure now reaches the summary rather than the run exiting 0 (#1145)
