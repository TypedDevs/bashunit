...
### Fixed
- The deprecation warning for the old `--assert` option quotes the invocation that was typed. `bashunit -a` reaches the deprecated path through the inferred `test` subcommand, so the warning named `bashunit test --assert`, a command the user never ran, and read like a warning about something else. It now prints `bashunit -a`, `bashunit --assert` or `bashunit test --assert` to match (#1366)
