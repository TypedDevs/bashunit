...
### Fixed
- The coverage differentials no longer flake under a loaded parallel suite: they compared the two implementations through `diff <(...) <(...)`, and when the process substitution failed with `diff: /dev/fd/N: Bad file descriptor` its stderr read as a rule disagreement naming an arbitrary file — the most alarming message those tests can produce, over nothing having disagreed (#1152)
