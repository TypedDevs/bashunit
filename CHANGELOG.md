...
### Fixed
- A bootstrap file that fails to load now reports it and exits non-zero, through every path that loads one (`BASHUNIT_BOOTSTRAP`, `--env`, `--boot`, and the `bench` equivalents), instead of leaving the run with no tests, no summary and exit 0. a syntax error makes `source` return non-zero, which ends the shell because `set -e` is active there, and a bare `exit` in the file ends it regardless — either way nothing after the call runs — an absent bootstrap is still ignored, since the default path is one most projects do not have (#1179, #1181)
