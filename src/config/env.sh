...
# An empty selection is a failure by default -- selecting nothing is usually a
# typo -- but a sharded CI matrix has shards with no tests in them on purpose.
# jest and vitest both spell the opt-out --passWithNoTests; this is that flag,
# kebab-cased. It never excuses a path that is not on disk: that is a wrong
# invocation, refused by name before discovery runs.
